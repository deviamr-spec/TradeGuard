...
class ScalpingStrategy:
    ...
    def calculate_stochastic(self, df: pd.DataFrame, k_period: int = 14, d_period: int = 3) -> Tuple[pd.Series, pd.Series]:
        ...
        try:
            if df is None or df.empty:
                raise IndicatorCalculationError("Empty DataFrame for Stochastic calculation")
            
            required_cols = ['high', 'low', 'close']
            missing_cols = [col for col in required_cols if col not in df.columns]
            if missing_cols:
                raise IndicatorCalculationError(f"Missing columns for Stochastic: {missing_cols}")
            
            if len(df) < k_period + d_period:
                raise IndicatorCalculationError(f"Insufficient data for Stochastic({k_period}, {d_period}): {len(df)} bars")
            ...
            if k_percent.isna().all() or d_percent.isna().all():
                raise IndicatorCalculationError("Stochastic calculation produced all NaN values")
            ...
            if isinstance(e, IndicatorCalculationError):
                raise
            raise IndicatorCalculationError(f"Stochastic calculation failed: {str(e)}")
    ...
    def generate_signal(self, df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            stoch_overbought = latest_stoch_k > 80 and latest_stoch_d > 80
            
            # Signal logic with confidence calculation
            confidence = 0.0
            
            # BUY signal conditions
            if ema_bullish and (rsi_oversold or (rsi_neutral and stoch_oversold)):
                signal_data["signal"] = "BUY"
                confidence += 40  # Base confidence for EMA bullish
                
                if rsi_oversold:
                    confidence += 30  # RSI oversold adds confidence
                if stoch_oversold:
                    confidence += 20  # Stochastic confirmation
                if ema_trend_strength > 0.1:
                    confidence += 10  # Strong trend
                    
            # SELL signal conditions
            elif not ema_bullish and (rsi_overbought or (rsi_neutral and stoch_overbought)):
                signal_data["signal"] = "SELL"
                confidence += 40  # Base confidence for EMA bearish
                
                if rsi_overbought:
                    confidence += 30  # RSI overbought adds confidence
                if stoch_overbought:
                    confidence += 20  # Stochastic confirmation
                if ema_trend_strength > 0.1:
                    confidence += 10  # Strong trend
